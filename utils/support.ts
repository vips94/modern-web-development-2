...
export const clamp = (val:any, min = 1, max = 10) =>
...
export const fix = () => {
  ...
  if (document.querySelector(s))
    document.querySelectorAll(s).forEach((e, i) => ((e as any).style.width = i != 3 ? "30%" : "50%"))
  if (document.querySelector(c)) {
    (document.querySelector(c)as any).parentElement.style.float = "none";
    (document.querySelector(c)as any).parentElement.style.width = "100% ";
  }
  if (document.querySelector(s + ".color"))
    (document.querySelector(s + ".color")as any).parentElement.style.width = "60%";
}

export const redraw = (elemMesh:any, v:any) => {
...
function getTranslate3d(el:any) {
  ...
  return values.map((c:any) => parseFloat(c.replace('px', '')))
}
...
export const init = (
    ...
    panel = controlKit
      ...
      .addButton("Save To Clipboard", () => {
        ...
        navigator.clipboard.writeText(stringify(rest)).then(
          () => {
            (document.querySelector('#controlKit .panel .button, #controlKit .picker .button') as any).value = "Copied Successfully";
            (document.querySelector('#controlKit .panel .button, #controlKit .picker .button') as any).style.pointerEvents = "none";
            (document.querySelector('body #controlKit .panel .button, #controlKit .picker .button') as any).style.backgroundColor = "darkgreen";
            (document.querySelector('body #controlKit .panel .button, #controlKit .picker .button') as any).style.color = "white";
            setTimeout(() => {
              (document.querySelector('#controlKit .panel .button, #controlKit .picker .button') as any).value = "Save To Clipboard";
              (document.querySelector('#controlKit .panel .button, #controlKit .picker .button') as any).style.pointerEvents = "initial";
              (document.querySelector('body #controlKit .panel .button, #controlKit .picker .button') as any).style.backgroundColor = "#383c4a";
              (document.querySelector('body #controlKit .panel .button, #controlKit .picker .button') as any).style.color = "#8c92a4";
            }, 1500)
          },
          () => {
            (document.querySelector('#controlKit .panel .button, #controlKit .picker .button')as any).value = "Failed To Copy";
            (document.querySelector('#controlKit .panel .button, #controlKit .picker .button')as any).style.pointerEvents = "none";
            (document.querySelector('body #controlKit .panel .button, #controlKit .picker .button')as any).style.backgroundColor = "red";
            (document.querySelector('body #controlKit .panel .button, #controlKit .picker .button')as any).style.color = "white";
            setTimeout(() => {
              (document.querySelector('#controlKit .panel .button, #controlKit .picker .button')as any).value = "Save To Clipboard";
              (document.querySelector('#controlKit .panel .button, #controlKit .picker .button')as any).style.pointerEvents = "initial";
  ...
  let mouseCoords = new THREE.Vector2(0.5, 0.5) as any

  let isMouseOverElemMesh = false

  const mover = (event:any) => {
    ...
    const intersects = raycaster.intersectObject(elemMesh) as any
  ...
  let loaded = false
  function createCroppedTexture(sources:any, newAspect:any, oldTextures: any) {
    
    return Promise.all(
      sources.map((source:any, index:any) => {
        function load(resolve:any) {
  ...
  function animate() {
    ...
    if (document.querySelector(o))
      if (parseInt((document.querySelector(o)as any).style.top) < 0)
        (document.querySelector(o) as any).style.top = "0px"
